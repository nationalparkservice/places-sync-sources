...
var esriRest = require('../helpers/esri-rest.js');
...
var runQuery = function (sourceUrl, queryObj, primaryKeys, sourceInfo, options) {
  // console.log('a');
  var getCount = {
  ...
  return postAsync(sourceUrl + 'query', getCount).then(function (countResult) {
    ...
    if (!countResult.error) {
      ...
      // }
      return esriRest(sourceUrl, queryObj, primaryKeys, sourceInfo, options).then(function (baseResults) {
        // console.log('d', baseResults);
...
var QuerySource = function (connectionString, sourceInfo, baseFilter, columns, fields) {
  var options = {'asGeoJSON': false, 'stringifyGeometry': true};
  return function (type, whereObj, returnColumns) {
    ...
    var createQueries = new CreateQueries(columns, keys.primaryKeys, keys.lastUpdatedField, keys.removedField, {'defaultWhere': null, 'emptyWhereClause': '1=1'});
    ...
    // TODO: This doesn't seem to work well with any verion of the REST API
    ...
    return runQuery(connectionString.sourceUrl, query, keys.primaryKeys, sourceInfo, options).then(function (result) {
      // TODO Map Fields
      // return mapFields.data.to(result, fields.mapped)
      // console.log('================================================');
      // console.log(connectionString.sourceUrl, keys.primaryKeys, result);
      // console.log('================================================');
